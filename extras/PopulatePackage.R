...
#
# This file is part of EmcMehtaDementiaModel
#
...
replaceName <- function(packageLocation = getwd(), 
  ...
  # rename files:
  #=====
  # file.path(packageLocation,"EmcMehtaDementiaModel.Rproj")
  # file.path(packageLocation,"R/EmcMehtaDementiaModel.R")
  filesToRename <- c("EmcMehtaDementiaModel.Rproj","R/EmcMehtaDementiaModel.R")
  for(f in filesToRename){
    ParallelLogger::logInfo(paste0('Renaming ', f))
    fnew <- gsub("EmcMehtaDementiaModel", packageName, f)
  ...
  for( f in filesToEdit ){
    ...
    x <- readLines(f)
    y <- gsub( "EmcMehtaDementiaModel", packageName, x )
    cat(y, file=f, sep="\n")
